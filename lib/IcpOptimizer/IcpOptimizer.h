...
public:
  ...
  //compute the rmse between movedPC an matchPC
  double computeRMSE(PointCloud a, PointCloud b);

  //compute the Objective function's value between movedPC an matchPC
  double computeObjectiveFunctionValue(PointCloud a, PointCloud b);
...
//compute the FPFH Descriptor
  Eigen::Matrix<double,Eigen::Dynamic,10*4> computedFpfhDescriptor(PointCloud a,int k);//k is the num of nearest neighbors.
